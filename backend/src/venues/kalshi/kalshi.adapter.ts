...
import { KALSHI_MARKETS } from "./fixtures/markets.js";
import { KALSHI_ORDER_BOOKS } from "./fixtures/order-books.js";
import { KALSHI_PRICE_HISTORY } from "./fixtures/price-history.js";
...
export class KalshiAdapter implements VenueAdapter {
  ...
  async getMarket(sourceMarketId: string): Promise<VenueMarket | null> {
    return (
      KALSHI_MARKETS.find((m) => m.sourceMarketId === sourceMarketId) ?? null
    );
  }
  ...
  async getOrderBook(
    ...
    return (
      KALSHI_ORDER_BOOKS.find((b) => b.sourceMarketId === sourceMarketId) ??
      null
    );
  }
  ...
  async getPriceHistory(
    ...
    const points = KALSHI_PRICE_HISTORY[key] ?? [];
