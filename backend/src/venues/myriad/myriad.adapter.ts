...
import { MYRIAD_MARKETS } from "./fixtures/markets.js";
import { MYRIAD_ORDER_BOOKS } from "./fixtures/order-books.js";
import { MYRIAD_PRICE_HISTORY } from "./fixtures/price-history.js";

@Injectable()
export class MyriadAdapter implements VenueAdapter {
  readonly venue: Venue = "myriad";

  async listMarkets(): Promise<VenueMarket[]> {
    return [...MYRIAD_MARKETS];
  }

  async getMarket(sourceMarketId: string): Promise<VenueMarket | null> {
    return (
      MYRIAD_MARKETS.find((m) => m.sourceMarketId === sourceMarketId) ?? null
    );
  }
  ...
  async getOrderBook(
    ...
    return (
      MYRIAD_ORDER_BOOKS.find((b) => b.sourceMarketId === sourceMarketId) ??
      null
    );
  }
  ...
  async getPriceHistory(
    ...
    const points = MYRIAD_PRICE_HISTORY[key] ?? [];
